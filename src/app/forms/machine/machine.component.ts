...
import { Component, CUSTOM_ELEMENTS_SCHEMA, EventEmitter, Input, OnChanges, OnDestroy, OnInit, Output, SimpleChanges } from '@angular/core';
...
export class MachineComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    Object.keys(this.machineForm.controls).forEach((controlName) => {
      this.machineForm.get(controlName)?.valueChanges
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['serverValidationErrors']) {
      ...
      if (this.serverValidationErrors && Object.keys(this.serverValidationErrors).length > 0) {
        const applyResult = applyServerValidationErrors(this.machineForm, this.serverValidationErrors);
