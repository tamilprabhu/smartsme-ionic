...
export class ReferenceService {
    ...
    getStates(): Observable<StateItem[]> {
        return this.http
            .get<StateItem[]>(`${this.API_URL}/states`, { headers: this.getHeaders() })
    ...
    getDistricts(stateId: number): Observable<DistrictItem[]> {
        return this.http
            .get<
                DistrictItem[]
            >(`${this.API_URL}/districts/${stateId}`, { headers: this.getHeaders() })
            .pipe(catchError((error) => throwError(() => error)));
    }

    getPincodes(pincode: string): Observable<PincodeItem[]> {
        return this.http
            .get<PincodeItem[]>(`${this.API_URL}/pincodes?pincode=${encodeURIComponent(pincode)}`, {
