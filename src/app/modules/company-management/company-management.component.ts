...
export class CompanyManagementComponent implements OnInit, OnDestroy {
        ...
        this.companyService.deleteCompany(company.companySequence!).subscribe({
            next: () => {
                this.companies = this.companies.filter(
                    (c) => c.companySequence !== company.companySequence,
                );
    ...
    handleFormSubmit(formData: Company) {
        ...
        if (this.formMode === 'create') {
            ...
            this.companyService
                .updateCompany(this.selectedCompany.companySequence!, formData)
                .subscribe({
                    next: (updatedCompany) => {
                        const index = this.companies.findIndex(
                            (c) => c.companySequence === this.selectedCompany?.companySequence,
                        );
                        if (index > -1) {
