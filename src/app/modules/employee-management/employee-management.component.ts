...
export class EmployeeManagementComponent implements OnInit, OnDestroy {
    ...
    getEmployeeRoleLabel(employee: EmployeeWithUserItem): string {
        const nestedRole = (employee.User ?? employee.user)?.UserRoles?.[0] as
            | EmployeeWithUserRole
            | undefined;
    ...
    onHeaderBackClick(): void {
        if (this.action === 'list') {
            this.router.navigate([
                this.launchedFromDashboard ? '/tabs/home' : '/tabs/profile-masters',
            ]);
            return;
        }
    ...
    async submit(): Promise<void> {
        ...
        const request$ =
            this.action === 'create'
                ...
                : this.employeeService.updateEmployeeRegistration(
                      this.selectedEmployeeId as number,
                      payload,
                  );
            ...
            next: (response) => {
                ...
                if (selectedDistrictName) {
                    const district = this.districts.find(
                        (item) =>
                            item.districtName.toLowerCase() === selectedDistrictName.toLowerCase(),
                    );
        ...
        this.employeeService.deleteEmployeeRegistration(employeeSequence).subscribe({
            next: async () => {
                this.employees = this.employees.filter(
                    (item) => item.employeeSequence !== employeeSequence,
                );
    ...
    private buildPayload(): CreateEmployeeWithUserPayload {
        ...
        const stateName =
            this.states.find((item) => item.id === Number(value.stateId))?.stateName ?? '';
        const districtName =
            this.districts.find((item) => item.id === Number(value.districtId))?.districtName ?? '';
