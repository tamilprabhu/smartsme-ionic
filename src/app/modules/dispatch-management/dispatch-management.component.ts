        ...
        this.dispatchService.deleteDispatch(dispatch.dispatchSequence).subscribe({
            next: () => {
                this.dispatches = this.dispatches.filter(
                    (d) => d.dispatchSequence !== dispatch.dispatchSequence,
                );
    ...
    handleFormSubmit(formData: Dispatch) {
        ...
        if (this.formMode === 'create') {
            ...
            this.dispatchService
                .updateDispatch(this.selectedDispatch.dispatchSequence, formData)
                .subscribe({
                    next: (updatedDispatch) => {
                        const index = this.dispatches.findIndex(
                            (d) => d.dispatchSequence === this.selectedDispatch?.dispatchSequence,
                        );
                        if (index > -1) {
