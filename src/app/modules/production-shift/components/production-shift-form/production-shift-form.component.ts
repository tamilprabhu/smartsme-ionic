...
} from 'src/app/components/lookup-search-modal/lookup-search-modal.component';
...
export class ProductionShiftFormComponent implements OnInit, OnChanges, OnDestroy {
    ...
    private resolveEmployeeSelection(
        field: 'operator1' | 'operator2' | 'operator3' | 'supervisor',
        roleNames: string[],
    ) {
        const value = this.form.get(field)?.value;
        if (!value) {
    ...
    private resolveLookup(
        field: 'orderId' | 'productId' | 'machineId',
        resource: 'order' | 'product' | 'machine',
    ) {
        const value = this.form.get(field)?.value;
        if (!value) {
            return;
