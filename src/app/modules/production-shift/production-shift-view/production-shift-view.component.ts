...
export class ProductionShiftViewComponent implements OnInit {
    ...
    private loadEmployeeNames(
        operator1?: number | null,
        operator2?: number | null,
        operator3?: number | null,
        supervisor?: number | null,
    ) {
        this.resolveEmployeeName(
            operator1,
            ['PRODUCTION_EMPLOYEE'],
            (label) => (this.operator1Name = label),
        );
        this.resolveEmployeeName(
            operator2,
            ['PRODUCTION_EMPLOYEE'],
            (label) => (this.operator2Name = label),
        );
        this.resolveEmployeeName(
            operator3,
            ['PRODUCTION_EMPLOYEE'],
            (label) => (this.operator3Name = label),
        );
        this.resolveEmployeeName(
            supervisor,
            ['SHIFT_INCHARGE'],
            (label) => (this.supervisorName = label),
        );
    }
