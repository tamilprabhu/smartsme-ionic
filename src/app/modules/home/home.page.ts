...
export class HomePage implements OnInit {
    ...
    private loadDashboard() {
        ...
        const products$ = this.canView('products')
            ? this.safe(
                  this.productService
                      .getProducts(1, ItemsPerPage.ONE)
                      .pipe(map((r) => r.paging.totalItems)),
              )
            : from([null]);
        const machines$ = this.canView('machines')
            ? this.safe(
                  this.machineService
                      .getMachines(1, ItemsPerPage.ONE)
                      .pipe(map((r) => r.paging.totalItems)),
              )
            : from([null]);
        ...
        const companies$ = this.canView('company')
            ? this.safe(
                  this.companyService
                      .getCompanies(1, ItemsPerPage.ONE)
                      .pipe(map((r) => r.paging.totalItems)),
              )
            : from([null]);
