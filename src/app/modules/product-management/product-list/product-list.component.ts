...
export class ProductListComponent implements OnInit, OnDestroy {
  ...
  loadProducts(event?: CustomEvent): void {
    ...
    this.productService.getProducts(this.currentPage, 10, this.searchControl.value, this.sortBy, this.sortOrder).subscribe({
      next: (response) => {
        this.products = this.currentPage === 1 ? response.items : [...this.products, ...response.items];
      ...
      error: (error) => {
        this.loading = false;
        this.listError = error?.error?.error || error?.message || 'Failed to load products';
        this.completeInfinite(event);
      }
    });
  }
